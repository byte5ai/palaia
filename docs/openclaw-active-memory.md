...
|---|---|---|
| `memory_search` | always (`tools.ts:71`) | `query`, `maxResults`, `tier`, `type` |
| `memory_get` | always (`tools.ts:192`) | `path`, `from`, `lines` |
| `memory_write` | opt-in — registered with `{ optional: true }` (`tools.ts:230,359`), so each agent must allow it | `content`, `scope`, `tags`, `type`, `project`, `title`, `force` |
...
out to internally (`tools.ts:141-148`). Anywhere a host config wants *tool names*, the
...
(`tools.ts:130`). When that path is unavailable, `memory_search` falls back to a
`palaia query … --json` subprocess with a **hard-coded 15 s timeout** that no config key
changes (`tools.ts:148`). So raising `timeoutMs` buys headroom on the fast path — useful
...
memory store has no equivalent for (`tools.ts:75-90`):
...
- **Scope isolation follows `PALAIA_AGENT`.** `memory_search` resolves scope visibility
  from the `PALAIA_AGENT` environment variable (`tools.ts:104`), not from the calling
