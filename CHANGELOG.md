...
### Fixed
- **OpenClaw `memory_search`: correct `maxResults` and `tier` descriptions** (#465) — `maxResults` advertised "default: 5" and carried a schema `default: 5`, while the tool actually falls back to the plugin's `maxResults` setting (10). A host filling in the schema default silently overrode the configured value. The schema default is gone and the description now defers to the plugin setting. `tier` is documented as what it is (`"all"` adds COLD entries; any other value searches HOT + WARM), the tool description states what it returns, and the README explains how the `tier` setting differs between searches and list-based recall.
