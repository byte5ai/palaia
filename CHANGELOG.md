...
### Added
- **OpenClaw: capture before compaction** (#185) — when OpenClaw compacts a long conversation, what it drops was never captured: auto-capture runs after each turn, and session summaries only on `/new`, `/restart` and session end. The plugin now saves a session summary tagged `pre-compaction` right before compaction, from both seams OpenClaw offers: the `before_compaction` hook (awaited by the host, registered on the ContextEngine and the legacy path alike) and palaia's own ContextEngine `compact()`. Because palaia owns compaction, OpenClaw fires the hook without messages, so the plugin keeps the last 200 conversation messages per session (from `afterTurn`) and summarises those; the hook and `compact()` share an in-flight guard and a 60-second cooldown, so one compaction writes one entry. A pre-compaction summary does not suppress the regular session-end summary. New option `captureOnCompaction` (default `true`); set it to `false` to turn this off. A failed capture never blocks compaction.
