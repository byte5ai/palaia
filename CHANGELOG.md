...
### Fixed
- **OpenClaw `memory_search` honours `tier` overrides from `priorities.json`, like recall** (#482) — auto-recall (`before_prompt_build` hook and ContextEngine) takes the tier from the resolved priorities, where a global or per-agent `tier` in `.palaia/priorities.json` overrides the plugin setting. `memory_search` read the plugin setting directly, so with an override of `"all"` recall included COLD entries while an explicit search from the same agent did not, and an override of `"hot"` for one agent was ignored under a plugin tier of `"all"`. It now includes COLD entries when the resolved tier is `"all"` (or when called with `tier: "all"`), and resolves with the configured `captureProject` as recall does. Project-level priorities carry only `blocked` and `recallTypeWeight`, so they do not change the tier on either path. If the priorities cannot be resolved, the search falls back to the plugin setting. The `tier` parameter description now points to the override.
- **OpenClaw `memory_search`/`memory_get`: numeric parameters accept only positive integers** (#483) — `maxResults`, `from` and `lines` were declared as plain numbers, so a host accepted `2.5`, `0` or `-3`. An explicit `maxResults: 0` silently became the default, `-3` reached the search as `top_k: -3` / `--limit -3`, and a fractional value failed in the CLI's argument parser instead of in schema validation the model can correct. All three are now integers with `minimum: 1`; `maxResults` also has a `maximum` of 100 (raised to the configured `maxResults` when that is larger; the search backend itself has no cap). Hosts that skip schema validation get the same check at runtime: an invalid value returns a message naming the accepted range and never reaches the CLI or the embed server. A configured `maxResults` that is not a positive integer falls back to the built-in default (10).
- **MCP server: `palaia_gc` reports what GC found and did** — the tool read a `moves` list that `Store.gc()` never returns, so tier moves were never reported and every dry run answered "nothing to do". A dry run now lists the scored entries, lowest GC score first (new `limit` parameter, default 20). It states whether a storage budget would prune anything, and that it predicts neither moves nor pruning. A real run reports the tier moves per direction, the entries pruned over the storage budget, and the cleanup counts. Entry titles are listed only for entries the server's agent may see; other agents' private entries are only counted.
- **MCP server acts as the configured agent** — the server never passed an agent identity to the store, so it could not read, search, list or edit even its own `private` entries, and `palaia_store` created entries without an owner although its `agent` parameter claimed auto-detection. The server now resolves its identity exactly like the CLI (`palaia.cli_helpers.resolve_agent_for_root`) and uses it in every tool. Other agents' private entries answer "Entry not found" in `palaia_read`/`palaia_edit`, and short ids resolve only among visible entries. `palaia_store` records the identity as the owner and refuses private entries it could not access. The private-write guard is shared with `palaia write` and now also catches a private project or global default scope, not only an explicit `--scope private`.
- **OpenClaw `memory_search`: correct `maxResults` and `tier` descriptions** (#465) — `maxResults` advertised "default: 5" and carried a schema `default: 5`, while the tool actually falls back to the plugin's `maxResults` setting (10). A host filling in the schema default silently overrode the configured value. The schema default is gone; both descriptions are now built from the resolved plugin config, so they state the configured `maxResults` and say when `tier` has no effect. `tier` is documented as what it is (`"all"` adds COLD entries; any other value searches HOT + WARM), the tool description states what it returns, and the README explains how the `tier` setting differs between searches and list-based recall.
- **MCP server: pin the MCP SDK to 1.x** — the `mcp` extra required `mcp>=1.0.0`, so a fresh `pip install 'palaia[mcp]'` resolved to mcp 2.x, which renamed `FastMCP` and removed `mcp.server.fastmcp`; `palaia-mcp` then failed at startup with a misleading "MCP SDK not installed". The extra is now `mcp>=1.2.0,<2` (1.2.0 is the first release with `FastMCP`). `palaia-mcp` and `palaia doctor` now check the installed version against that range. For an out-of-range SDK they name the installed version and the supported range, where doctor used to report the MCP server as ok. A supported SDK that still fails to import is reported with its real import error.
- **Search: private entries reach their owner, filters apply to vector hits** — `SearchEngine.search()` checked scope while building its index, but re-read every hit without the agent, which dropped all private entries even for their owner (in `palaia query` and the embed server). Native vector search (sqlite-vec / pgvector) scores the whole embedding table, so its hits bypassed the project/type/status/tier filters. Only entries that pass scope and filters are scored now. The native path pushes the type filter down and over-fetches until it has enough eligible hits, so filtered-out neighbours can no longer crowd out eligible semantic matches. The re-read uses the same agent and scope visibility.
...
- **`memory_write` duplicate guard works, and names the real retry parameter** (#466) — the guard is meant to block a write when a near-duplicate was created in the last 24 hours, but it read the creation time from `created_at`/`createdAt`/`date`, none of which `palaia query --json` returned, so it never fired. Search results now include the entry's `created` timestamp (direct and embed-server paths alike), and the plugin reads that field. The guard also judged similarity by the ranking `score`, which palaia normalizes to the best hit — with BM25-only search the top hit always scores 1.0, so a live guard would have blocked any write sharing a word with a recent entry. It now uses `embed_score`, the raw cosine similarity, and blocks above 0.89, comparing the new entry as palaia indexes it — title, tags and content, not the content alone, which scored titled near-duplicates below any usable threshold (calibrated with the default fastembed model: near-duplicates score 0.90–0.97, paraphrases 0.84–0.88); without an embedding provider the check is skipped. The guard now asks the warm embed server first, like `memory_search`, instead of spawning a CLI process whose 2-second budget a cold embed server outlasted. The embed server now checks for entries added, removed or moved by other processes before each query, status and warmup request, and then rebuilds its index and reloads its embedding cache. This replaces its 30-second background check, which left an entry written moments earlier invisible to the guard and `memory_search`, and on every change also discarded the loaded embedding model. Its reply told the model to "confirm with `--force`", a CLI flag the tool does not accept; it now says "call again with force: true", and the tool description documents the guard, the retry path, and that the tool is meant for processes/SOPs and tasks. Side effect: the Age column of `palaia query` is now filled in. With an older palaia CLI the guard stays inactive, as before; an already-running embed server needs a restart to return `created`.
