...
- **`memory_write` duplicate guard works, and names the real retry parameter** (#466) — the guard is meant to block a write when a similar entry (score > 0.8) was created in the last 24 hours, but it read the creation time from `created_at`/`createdAt`/`date`, none of which `palaia query --json` returned, so it never fired. Search results now include the entry's `created` timestamp (direct and embed-server paths alike), and the plugin reads that field. The guard's reply also told the model to "confirm with `--force`", a CLI flag the tool does not accept; it now says "call again with force: true", and the tool description documents the guard, the retry path, and that the tool is meant for processes/SOPs and tasks. Side effect: the Age column of `palaia query` is now filled in. Limits: the guard only fires when its duplicate query answers within the plugin's 2-second budget — with an embedding chain configured and no embed server running yet, the query waits for the server to start and the guard lets the write through, as it always has on timeout. With an older palaia CLI the guard stays inactive, as before; an already-running embed server needs a restart to return `created`.
