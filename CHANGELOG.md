...
- **`memory_write` duplicate guard names the real retry parameter** (#466) — when a similar entry (score > 0.8) from the last 24 hours blocked a write, the reply told the model to "confirm with `--force`", a CLI flag the tool does not accept; a model following it could not retry. The reply now says "call again with force: true", and the tool description documents the guard, the retry path, and that the tool is meant for processes/SOPs and tasks.
