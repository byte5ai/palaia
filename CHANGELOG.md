...
### Fixed
- **MCP server acts as the configured agent** — the server never passed an agent identity to the store, so it could not read, search, list or edit even its own `private` entries, and `palaia_store` created entries without an owner although its `agent` parameter claimed auto-detection. The server now resolves its identity at startup (`PALAIA_AGENT`, else `agent` in the config) and uses it in every tool. `palaia_store` records that identity as the owner, and it refuses a `private` entry when there is no identity, since nobody could access that entry.
- **Search returned no private entries, even to their owner** — `SearchEngine.search()` checked scope while building the index, then re-read each hit without the agent, which dropped every private entry. This affected `palaia query` too. The re-read now uses the same agent and scope visibility.
