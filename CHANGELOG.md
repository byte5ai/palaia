...
### Fixed
- **`palaia write --type process`: "Similar process found" only for similar processes** (#481) — the nudge warned when a hit's ranking `score` exceeded 0.8, but that score is normalized to the best hit: in a BM25-only store the top hit always scores 1.0, so any new process sharing a single word with an existing one was told to edit that unrelated process instead. The nudge now judges by `embed_score`, the raw cosine similarity, and fires at 0.85 or above (with the default fastembed model paraphrases score 0.84–0.88, same-topic but different content 0.75–0.83); it names the most similar process rather than the top-ranked one and reports its similarity. Without an embedding provider it stays silent. It compares the new entry as palaia indexes it — title (auto-extracted when none is given), tags and content — instead of the content alone.
