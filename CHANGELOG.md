...
- **Search: private entries reach their owner, filters apply to vector hits** — `SearchEngine.search()` checked scope while building its index, but re-read every hit without the agent, which dropped all private entries even for their owner (this affected `palaia query`, the embed server and MCP). Native vector search (sqlite-vec / pgvector) scores the whole embedding table, so its hits bypassed the project/type/status/tier filters. Only entries that pass scope and filters are ranked now, and the re-read uses the same agent and scope visibility.
