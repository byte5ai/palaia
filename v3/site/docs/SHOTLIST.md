...
| 3 | `memory.md` | A pending review proposal on the dashboard's Review queue page (Memory → Review queue), its details expanded, with the Approve/Reject buttons (SPEC-206 policy, SPEC-208 review queue) | Needs a real merge/rename/retire proposal to exist first — the automated-capture path (SPEC-207) is the easiest way to generate one worth screenshotting |
