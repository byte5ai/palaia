...
`telegram.bot.state`, by each bot's long poller through it). Three of the
four message events carry **metadata only, never the message text** — a dump of
...
| `telegram.bot.state` | `telegram` | `bot`, `state` (`ok` or `failing`), `detail` (the error line, token scrubbed; empty on recovery) | A polling bot's poll starts failing, or recovers — once per transition, never once per poll (issue #439). It is what lets the dashboard's Telegram panel show a failing bot without polling the hub |
