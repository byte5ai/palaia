...
destination through the running hub — `POST /api/backup/targets/{name}/run`
or the backup schedule (issue #438, `backup.interval_hours`), both through
`palaia_hub.backup_targets.run_target`. The owner decision behind them:
*backup failures surface as events/notifications, never silently* — and the
success event is what makes "my backups quietly stopped" visible at all.
Neither carries a path from inside the archive or anything about its
contents. Both carry `trigger` — `manual` for the dashboard's run action,
`schedule` for the timer — so an automation can route "the nightly backup
failed" differently from "the one I just clicked failed".
