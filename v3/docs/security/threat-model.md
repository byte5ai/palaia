...
| `/api/backup/targets` | lists the destinations this hub writes that same archive to itself — with each one's last outcome and the schedule (issue #438) — and runs one on demand (issue #297) — `GET` lists, `POST /api/backup/targets/{name}/run` writes | the response never carries archive bytes (they go hub → the operator's own directory), but naming a destination is naming where the keys go: same admin gate, and the same 403-with-the-CLI-named refusal on an ungated hub. The schedule itself is set only in `config.yaml` — nothing on this route changes it |
...
| The full archive — secret store *and* its key — sent to a destination nobody vetted for key material (issue #297) | A backup target declares at class level whether it moves the full archive and whether its destination is secret-safe; `BackupTarget.run` refuses the mismatch **before building a byte**, so the rule is an invariant rather than a comment a future notes-only git target could be written around | `server/src/palaia_hub/backup_targets.py`, `server/tests/backup/test_targets.py::test_a_target_that_would_move_the_full_archive_somewhere_unsafe_is_refused` |
| A hub backing itself up into its own home — every archive nesting the last, all of them lost with that one disk | The local-directory target resolves its destination against the hub home on every run and refuses one inside it, naming the fix | `server/src/palaia_hub/backup_targets.py`, `server/tests/backup/test_targets.py::test_a_directory_inside_the_hub_home_is_refused` |
| Two runs of one target at once — a scheduled one and a click (issue #438) — writing the same second-stamped `.part` file into each other | Every run by the running hub goes through one ledger holding a lock per target: a click on a busy target answers 409, a scheduled pass skips it, and passes never overlap. The status file the schedule keeps (`backup-status.json`) holds names, times, file names and failure reasons only — nothing from inside an archive — and a damaged one is read as "never ran" rather than stopping the hub | `server/src/palaia_hub/backup_schedule.py`, `server/tests/backup/test_schedule.py`, `server/tests/backup/test_routes.py::test_a_target_already_being_written_answers_409_and_writes_nothing` |
| An archive written to an operator's directory left readable by anyone else on the box, or a killed run leaving a truncated file that looks restorable | Written through a `.part` sibling, `chmod 0600` **before** the rename into place; retention only ever considers files matching the archive naming, so nothing the operator put in that directory is touched | `server/src/palaia_hub/backup_targets.py`, `server/tests/backup/test_targets.py` |
