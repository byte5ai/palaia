...
| `/api/backup/targets` | lists the destinations this hub writes that same archive to itself, and runs one on demand (issue #297) — `GET` lists, `POST /api/backup/targets/{name}/run` writes | the response never carries archive bytes (they go hub → the operator's own directory), but naming a destination is naming where the keys go: same admin gate, and the same 403-with-the-CLI-named refusal on an ungated hub |
